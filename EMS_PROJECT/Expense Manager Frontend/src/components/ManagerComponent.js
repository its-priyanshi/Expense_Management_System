...
function ManagerComponent(){
    ...
    const search = (e) => {
        ...
        const searchResult = expenses.filter((post) => {
        return (
    ...
    const addExpense=()=>{
        navigate(`/managerAddExpense/${id}/${name}/${jobtitle}`)
    }
    const actOnExpense=()=>{
        navigate(`/actionExpense/${id}/${name}/${jobtitle}`)
    }
    const getAnalytics=()=>{
        navigate(`/managerAnalytics/${id}/${name}/${jobtitle}`)
    }
    const getMyTeam=()=>{
        navigate(`/getSubordinates/${id}/${name}/${jobtitle}`)
    }
        ...
        <>
            <div>
                <div class="container-fluid d-flex align-items-center justify-content-between pb-0 mt-0 mb-0 border-bottom text-white" style={{"background-color" : "#121C4E"}}>
                    <img src={ADP_HomePage_logo} className="img-fluid pt-3 "  width={100} height={30} alt="adp logo"/>
            ...
            <div className = "container mb-5">
                    ...
                    <nav class="navbar navbar-light bg-light">
                        <form class="container-fluid justify-content-start">
                            <button class="btn btn-secondary me-2" type="button" onClick={addExpense}>Add Expense</button>
                            <button class="btn btn-secondary me-2" type="button" onClick={getAnalytics}>My Analytics</button>
                            <button class="btn btn-secondary me-2" type="button" onClick={actOnExpense}>Action on Expense</button>
                            <button class="btn btn-secondary" type="button" onClick={getMyTeam}>My team</button>
                        </form>
                    </nav>
