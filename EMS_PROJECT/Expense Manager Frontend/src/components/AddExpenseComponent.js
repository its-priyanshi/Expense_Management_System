            ...
            if(typeError==='' &&  receiptError==='' && amountError==='')
                ...
                uploadTransaction({"associateId":id,"expenseType":expenseType,"startDate":startDate,"endDate":endDate,"claimAmount":claimAmount,"receipt":receipt}).then(
                    (response)=>{
                        ...
                        navigate(`/sampleDashboard/${id}/${name}/${jobtitle}/${isManager}`)
                    }
        ...
        <>
            ...
            <div className='py-1'>
                <ul class="nav nav-tabs">
                    <li class="nav-item">
                    <Link to={`/sampleDashboard/${id}/${name}/${jobtitle}/${isManager}`} exact component={EmployeeComponent} className="nav-link"> Home </Link> 
                    </li>
                    <li class="nav-item">
                        <a class="nav-link active">Add Expense</a>
                    </li>
                    <li class="nav-item">
                    <Link to={`/employeeAnalytics/${id}/${name}/${jobtitle}/${isManager}`} exact component={AddExpenseComponent} className="nav-link"> My analytics </Link> 
                    </li>
                </ul>
            </div>
            <div className = "container">
                <div className = "row">
                    <div className = "card col-md-6 offset-md-3 offset-md-3 mb-2">
            ...
            <footer className="d-flex justify-content-end position-sticky mt-2 bottom-0 end-0 w-100" style={{"height" : "40px" , "background-color" : "#E3DFDA"}}>
            <div className="d-flex align-items-center justify-content-center pe-4">
                <p className="text-dark text-center pt-3">Copyright © 2023 ADP, Inc.</p>
            </div>
            </footer>
        </>
    )
